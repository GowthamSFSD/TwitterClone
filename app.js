...
// getting user following epople id
...
///api 3 API 3 Path: /user/tweets/feed/ Method: GET
app.get("/user/tweets/feed/", authentication, async (request, response) => {
  const { username } = request;
  const followingPeoplesIds = await getFollowingPeopleIdsOfUser(username);

  const getTweeetsFeedQuery = `SELECT 
  username,tweet,date_time AS dateTime 
FROM  user INNER JOIN tweet ON user.user_id = tweet.user_id
WHERE  user.user_id IN (${followingPeoplesIds})
ORDER BY date_time DESC
LIMIT 4;  
`;
  const tweetFeedArray = await db.all(getTweeetsFeedQuery);
